...
function CommentNotification({ onClose }) {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await fetch("http://13.50.175.179:5000/api/comments");
  ...
  const handleDelete = async (commentId) => {
    try {
      await fetch(`http://13.50.175.179:5000/api/comments/${commentId}`, {
        method: "DELETE",
      });
          ...
          {comments.map((comment) => (
            <li key={comment._id}>
              ...
              <button
                onClick={() =>
                  alert(
                    comment.commentImage ? comment.commentImage : "No image"
                  )
                }
              >
                View Details
