...
const dualColorBarsPlugin = {
  ...
  beforeDatasetsDraw(chart, args, options) {
    ...
    chart.getDatasetMeta(0).data.forEach((bar, index) => {
      ...
      console.log(
        `Drawing bar for index ${index} with colors ${leftColor} and ${rightColor}`
      );
      console.log(
        `Bar coordinates: x=${bar.x}, y=${bar.y}, height=${barHeight}, width=${bar.width}`
      );
...
function PerformanceMetrics() {
  ...
  const fetchData = async () => {
    try {
      ...
      const tripsResponse = await fetch("http://13.50.175.179:5000/api/trips");
      ...
      trips.forEach((trip) => {
        const vehicle = vehicles.find(
          (v) => v.vehicleNumber === trip.vehicleNumber
        );
        if (vehicle) {
