...
function SosNotification({ type, onClose }) {
    ...
    const fetchIssues = async () => {
      try {
        const response = await fetch(
          `http://13.50.175.179:5000/api/issues?type=${type}`
        );
  ...
  const handleDelete = async (issueId) => {
    try {
      await fetch(`http://13.50.175.179:5000/api/issues/${issueId}`, {
        method: "DELETE",
      });
    ...
    <div className="sos-notification">
      <div className="sos-notification-content">
        <div className="notification-header">
          <h2>{type === "sos" ? "SOS NOTIFICATION" : "TRIP NOTIFICATION"}</h2>
        ...
        <ul>
          {issues.map((issue) => (
            <li key={issue._id}>
              ...
              <button
                className="more-button-sos"
                onClick={() =>
                  alert(issue.issueImage ? issue.issueImage : "No image")
                }
              >
                View Details
              </button>
