      ...
      const fetchVehicleData = async () => {
        try {
          const response = await fetch(
            `http://13.50.175.179:5000/api/vehicles?vehicleNumber=${trip.vehicleNumber}`
          );
    ...
    <div className="trips-container">
      ...
      <div className="trip-display">
        <div className="car-image">
          <div className="arrow left-arrow" onClick={handlePreviousTrip}>
            &lt;
          </div>
          <img src={carImage} alt="Car" />
          {trips.length > 0 && <TripDetails trip={trips[currentTripIndex]} />}
          <div className="arrow right-arrow" onClick={handleNextTrip}>
            &gt;
          </div>
        </div>
      </div>
