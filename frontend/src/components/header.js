    ...
    const fetchManagerUsername = async () => {
      try {
        const response = await fetch(
          "http://13.50.175.179:5000/api/getManagerUsername"
        );
    ...
    const fetchSosAndCommentNotifications = async () => {
      try {
        const sosResponse = await fetch(
          "http://13.50.175.179:5000/api/issues?type=sos"
        );
        const sosData = await sosResponse.json();
        setHasSosNotifications(sosData.issues.length > 0);

        const commentResponse = await fetch(
          "http://13.50.175.179:5000/api/issues?type=comment"
        );
  ...
  return (
    <header className="headero">
      <div className="header-org">
        <h1>
          WELCOME TO <span>247 FLEET MANAGER</span>
        </h1>
        ...
        <div className="profile-container">
          ...
          {showDropdown && (
            <div className="dropdown-menu">
              ...
              <Link to="/" className="dropdown-item-button">
                Logout
              </Link>
            </div>
          )}
