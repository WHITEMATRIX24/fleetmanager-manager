...
function ViewTrips() {
  ...
  useEffect(() => {
    axios
      .get("http://13.50.175.179:5000/api/allTrips")
      .then((response) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(
        `http://13.50.175.179:5000/api/trips/${selectedTrip._id}/extend`,
        {
          method: "POST",
  ...
  const formatDates = (startDate, endDate) => {
    const formattedStart = new Date(startDate).toLocaleDateString(undefined, {
      day: "numeric",
      month: "short",
      year: "numeric",
    });
    const formattedEnd = new Date(endDate).toLocaleDateString(undefined, {
      day: "numeric",
      month: "short",
      year: "numeric",
    });
  ...
  return (
    <div className="view-trip-main">
      ...
      <div className="view-trip-content">
                ...
                {filteredTrips.map((trip) => (
                  <tr key={trip._id}>
                    ...
                    <td style={{ whiteSpace: "pre-line" }}>
                      {formatDates(trip.tripDate, trip.tripEndDate)}
                    </td>
                    <td style={{ whiteSpace: "pre-line" }}>
                      {formatLocation(
        ...
        {selectedTrip && (
          <div
            className="popup-form-overlay-veh"
            onClick={() => setSelectedTrip(null)}
          >
            <div
              className="popup-form-container-veh"
