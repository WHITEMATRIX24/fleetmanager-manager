...
function PopupForm({ car, onClose, onSubmit }) {
  ...
  useEffect(() => {
    if (car) {
      ...
      setInsuranceDueDate(
        car.insuranceDueDate
          ? new Date(car.insuranceDueDate).toISOString().split("T")[0]
          : ""
      );
      setIstimaraDueDate(
        car.istimaraDueDate
          ? new Date(car.istimaraDueDate).toISOString().split("T")[0]
          : ""
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://13.50.175.179:5000/api/updatevehicle`,
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="popup-form-overlay-veh">
      <div className="popup-form-container-veh">
        <span className="close-popup-veh" onClick={onClose}>
          &times;
        </span>
