...
function AssignTrip({ vehicleNo }) {
  ...
  const generateTripNumber = async () => {
    let tempID = lastTripID
      ? parseInt(lastTripID.replace("TR", ""), 10) + 1
      : 1;
    ...
    try {
      ...
      while (exists) {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to check trip ID. Status: ${response.status}`
          );
        }
        const data = await response.json();
        if (data.exists) {
  ...
  //   vehicle number passing
  //  re-rendering useEffect
  useEffect(() => {
    if (vehicleNo) setTripData({ ...tripData, vehicleNumber: vehicleNo });
  }, [vehicleNo]);
          ...
          <div class="add-trip">
            <div class="form-container">
              <form className="c-form-car" onSubmit={handleTripSubmit}>
                ...
                <div className="c-form-field-row-trip">
                  <div
                    className="c-form-field-column-trip"
                    style={{ width: "33%" }}
                  >
